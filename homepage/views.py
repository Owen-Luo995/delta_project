    ...
    with zipfile.ZipFile(rti_file_path, 'r') as zip_ref:
        for fileInfo in zip_ref.infolist():
            ...
            with zip_ref.open(fileInfo) as file:
                ...
                if fileName.lower() == 'info.json':
                    infojson = ContentFile(file.read(), name=fileName)
                    rti_instance.Info_File.save(uploadPath, infojson)
                elif file_extension.lower() == '.dzi':
                    if not rti_instance.Has_DZI:
                        rti_instance.Has_DZI = True
                elif file_extension.lower() == '.jpg':
                    if not rti_instance.Has_Image:
                        rti_instance.Has_Image = True
                elif file_extension.lower() == '.tzi':
                    if not rti_instance.Has_TZI:
                        rti_instance.Has_TZI = True
        ...
        for file_name in files:
            ...
            with open(file_path, 'rb') as file:
                if file_name.lower() == 'info.json':
                    infojson = ContentFile(file.read(), name=file_name)
                    rti_instance.Info_File.save(file_path, infojson)
                elif file_extension.lower() == '.jpg':
                    if not rti_instance.Has_Image:
                        rti_instance.Has_Image = True
...
def uploadItemFunction(
        ...
        accessionNumber, bibliography, details: list, rtiComp=None, rtiFilePath=None,
) -> Item:
    ...
    if rtiFilePath is not None and rtiComp != '':
        ...
        rtiInstance.RTI_Zip = rtiComp
        file = ""
        # Different processing for different file types(.zip or .ptm/.hsh)
        # If it is zip file, call the function to extract and save it directly
        if zipfile.is_zipfile(rtiFilePath):
            rtiInstance = zip_file_handler(rtiFilePath, title, rtiInstance)
            rtiInstance.save()
        # If it is a .ptm/.hsh file, use relight-cli to convert it first before save it.
        # Linux: 'linux'/'linux2', Mac: 'darwin', Windows: 'win32'
        elif sys.platform.startswith("win32"):
            output_folder = os.path.join(settings.MEDIA_ROOT, title)
            with open(rtiFilePath, 'r', encoding='utf-8'):
                try:
                    sub_result = subprocess.run([settings.RELIGHTLAB_DIR, rtiFilePath, output_folder],
                                                capture_output=True, text=True)
                    if sub_result.returncode == 0:
                        print(f"Conversion successful: {file} -> {output_folder}")
                    else:
                        print(f"Error during conversion: ")
                        print(sub_result.stderr)
                except subprocess.CalledProcessError as e:
                    print(f"Error during conversion: {e}")
                rtiInstance = ptm_hsh_file_handler(output_folder, title, rtiInstance)
                rtiInstance.save()
        else:
...
def home(request):
    ...
    results = Item.objects.filter(approve_status=1)  # Set Upload_Date to ensure result consistence
...
def rti_page(request, pk):
    try:
        ...
        file_type = ''
        planes = RTI_item.planes
        planes_url = []
        zip_path = RTI_item.RTI_Zip.path
        zip_path = os.path.join(settings.MEDIA_ROOT, zip_path)
        # At the top of the detailpage there is a link to view the rti
        extract_dir = os.path.dirname(zip_path)

        # Files that expect to be extracted
        files_to_check = ["info.json"]
        files_exist = all(os.path.exists(os.path.join(extract_dir, f)) for f in files_to_check)

        if not files_exist:
            temp_extract_dir = os.path.join(extract_dir, 'temp_extracted')
                ...
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    ...
                    for item in os.listdir(temp_extract_dir):
                        ...
                        if os.path.isdir(item_path):
                            target_path = os.path.join(extract_dir, item)
                            if os.path.exists(target_path):
                                shutil.rmtree(target_path)
                            shutil.move(item_path, extract_dir)
                        else:
                            shutil.move(item_path, extract_dir)
        ...
        if RTI_item.Has_DZI:
            file_type = "deepzoom"
        elif RTI_item.Has_TZI:
            file_type = "tarzoom"
        elif RTI_item.Has_Image:
            file_type = "image"
        else:
            raise Http404("No valid RTI file type found")

        for i in planes.all():
            planes_url.append(i.File.url)
        return render(request, 'rti.html',
                      {'planes_url': planes_url, 'rti': RTI_item, 'planes': planes, 'layout': file_type})
    except Http404 as e:
...
def review_items(request):
    # pending_items = Item.objects.filter(approve_status=0)
    ...
    # Set pagination for items
    items_per_page = 10
    paginator_items = Paginator(pending_items_list, items_per_page)
    page_number_items = request.GET.get('page_items')
    pending_items = paginator_items.get_page(page_number_items)

    # Set pagination for users
    users_per_page = 10
    paginator_users = Paginator(pending_users_list, users_per_page)
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save(commit=False)  # Do not commit to the database yet in order to set is_active to False

            # Set user attributes based on form data
            user.is_active = False  # 设置用户为非激活状态，等待管理员审核
            user.is_staff = form.cleaned_data.get('is_uploader', False)  # 使用is_uploader来决定is_staff
...
def user_item(request):
    ...
    items = Item.objects.filter(Uploader=request.user).order_by('-Upload_Date')  # Assuming 'uploader' is the field associated with the user
